...
import MementoDemo from './client';

// Raw code
import mementoCode from './memento.ts?raw';
import originatorCode from './originator.ts?raw';
import caretakerCode from './care-taker.ts?raw';
import clientCode from './client.tsx?raw';

export default {
  title: 'Design Patterns/Behavioral/Memento Pattern',
  parameters: {
    docs: {
      description: {
        component:
...
export const Implementation: StoryFn = () => (
  <Box className="space-y-8" p="3">
    ...
    <Flex align="center" gap="3" className="flex-wrap">
      <h3 className="text-2xl font-semibold tracking-tight">Memento Pattern</h3>
      <Badge variant="soft" color="cyan">Comportamental</Badge>
    </Flex>

    {/* Resumen */}
    <p className="text-sm leading-relaxed text-gray-600 dark:text-gray-300">
      <strong>Memento</strong> permite guardar snapshots del estado y restaurarlos cuando sea necesario, manteniendo encapsulado el estado interno del originador.
    </p>
    ...
    <section className="grid md:grid-cols-2 gap-6">
      <div className="rounded-xl border border-amber-200 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/40 p-5 shadow-sm space-y-3">
        ...
        <p className="text-xs sm:text-sm text-amber-900 dark:text-amber-200">
          Implementar deshacer/rehacer puede romper encapsulamiento si se expone todo el estado. Además, gestionar historial manual se vuelve complejo.
        </p>
      </div>
      <div className="rounded-xl border border-emerald-200 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-950/40 p-5 shadow-sm space-y-3">
        ...
        <p className="text-xs sm:text-sm text-emerald-900 dark:text-emerald-200">
          El <strong>Originador</strong> crea <em>Mementos</em> con su estado. El <strong>Caretaker</strong> guarda el historial y gestiona deshacer/rehacer sin tocar detalles internos.
        </p>
      </div>
    </section>
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-zinc-900 p-5">
					...
					<figure>
						<img src="/img/real_memento.png" alt="Memento UML Diagram" className="w-full h-auto rounded-md border border-gray-100 dark:border-gray-700 shadow-sm" loading="lazy" />
					</figure>
				</div>
    ...
    <section className="rounded-2xl border border-gray-300 dark:border-gray-700/80 bg-white dark:bg-zinc-900/70 p-6 shadow-sm space-y-8">
      <header className="flex items-center gap-3 flex-wrap">
        <h4 className="text-lg font-semibold tracking-tight">Caso Específico: Editor con Checkpoints</h4>
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white/70 dark:bg-zinc-800/60 p-5 space-y-3">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-300">
          Un editor de texto que guarda snapshots del contenido. Se puede deshacer al estado anterior o rehacer, y crear nuevos checkpoints que anulan el rehacer.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-zinc-900 p-5">
					<h5 className="text-sm font-semibold uppercase tracking-wide text-gray-500 mb-3">Diagrama UML</h5>
					<figure>
						<img src="/img/memento.png" alt="Memento UML Diagram" className="w-full h-auto rounded-md border border-gray-100 dark:border-gray-700 shadow-sm" loading="lazy" />
					</figure>
				</div>
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-zinc-900 p-5 space-y-5">
        ...
        <details className="group rounded-lg border bg-white dark:bg-zinc-800 open:shadow-inner">
          <summary className="cursor-pointer select-none text-sm font-medium px-4 py-2 bg-gradient-to-r from-indigo-500/10 to-violet-500/10 dark:from-indigo-400/10 dark:to-violet-400/10 border-b flex items-center gap-2">
            <span className="text-indigo-600 dark:text-indigo-300 group-open:font-semibold">Ver originador, memento, caretaker y cliente</span>
          ...
          <div className="p-4 space-y-4">
            <CodeBlock code={originatorCode} title="originator.ts" />
            <CodeBlock code={mementoCode} title="memento.ts" />
            <CodeBlock code={caretakerCode} title="care-taker.ts" />
...
export const Playground: StoryFn = () => (
  <div className="space-y-5">
    <div className="rounded-md border border-indigo-200 dark:border-indigo-800 bg-indigo-50/70 dark:bg-indigo-950/30 p-3 text-[12px] leading-relaxed text-indigo-900 dark:text-indigo-200">
      <strong>Cómo interactuar:</strong> Escribe en el editor, pulsa “Guardar checkpoint”, y usa “Deshacer”/“Rehacer”. Al crear un nuevo checkpoint, el historial de rehacer se limpia.
    </div>
    <MementoDemo />
  </div>
);
...
Playground.parameters = {
  docs: {
    description: {
      story: 'El Caretaker gestiona pilas de deshacer/rehacer y restaura mementos creados por el Originador, preservando el encapsulamiento.'
    }
  }
};
