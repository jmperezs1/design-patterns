...
import FlyweightDemoRadix from './client';

// Raw sources
import ifaceCode from './flyweight-interface.tsx?raw';
import concreteCode from './concrete-flyweight.tsx?raw';
import factoryCode from './flyweight-factory.tsx?raw';
import makeDataCode from './helpers/make-data.ts?raw';
import clientCode from './client.tsx?raw';

export default {
  title: 'Design Patterns/Structural/Flyweight Pattern',
  parameters: {
    docs: {
      description: {
        component:
          'Flyweight comparte estado intrínseco entre muchos objetos livianos para ahorrar memoria y mejorar desempeño. El estado extrínseco se pasa en cada operación, evitando duplicidad de datos por instancia.'
      }
    }
  }
} as Meta;
  ...
  return (
    <Box className="space-y-8" p="3">
      ...
      <Flex align="center" gap="3" className="flex-wrap">
        <h3 className="text-2xl font-semibold tracking-tight">Flyweight Pattern</h3>
        <Badge variant="soft" color="cyan">Estructural</Badge>
      </Flex>

      {/* Resumen */}
      <p className="text-sm leading-relaxed text-gray-600 dark:text-gray-300">
        <strong>Flyweight</strong> reduce el costo de memoria cuando hay muchísimas instancias similares.
      </p>
      ...
      <section className="grid md:grid-cols-2 gap-6">
        <div className="rounded-xl border border-amber-200 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/40 p-5 shadow-sm space-y-3">
          ...
          <p className="text-xs sm:text-sm text-amber-900 dark:text-amber-200">
            Dibujar miles de elementos con el mismo estilo implica crear muchas copias redundantes, consumiendo memoria y tiempo de render.
          </p>
        </div>
        <div className="rounded-xl border border-emerald-200 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-950/40 p-5 shadow-sm space-y-3">
          ...
          <p className="text-xs sm:text-sm text-emerald-900 dark:text-emerald-200">
            Compartir el estado inmutable común en objetos <em>flyweight</em> reutilizables, y pasar el estado cambiante en cada operación.
          </p>
        </div>
      </section>
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-zinc-900 p-5">
					...
					<figure>
						<img src="/img/real_flyweight.png" alt="Flyweight UML Diagram" className="w-full h-auto rounded-md border border-gray-100 dark:border-gray-700 shadow-sm" loading="lazy" />
					</figure>
        	</div>
      ...
      <section className="rounded-2xl border border-gray-300 dark:border-gray-700/80 bg-white dark:bg-zinc-900/70 p-6 shadow-sm space-y-8">
        <header className="flex items-center gap-3 flex-wrap">
          <h4 className="text-lg font-semibold tracking-tight">Caso Específico: Badges posicionados</h4>
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white/70 dark:bg-zinc-800/60 p-5 space-y-3">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Renderizar miles de badges con el mismo estilo pero posiciones y textos distintos saturaría memoria si se duplican estilos por cada badge.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-indigo-300 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-950/40 p-5 space-y-3">
          ...
          <p className="text-sm text-indigo-900 dark:text-indigo-200">
            La <code>BadgeFactory</code> crea y cachea flyweights por variante (pill, rounded-outline, chip) con su estilo intrínseco. Por cada item, se pasa el estado extrínseco (<code>text, x, y, color</code>) a <code>operation()</code>.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-zinc-900 p-5">
					<h5 className="text-sm font-semibold uppercase tracking-wide text-gray-500 mb-3">Diagrama UML</h5>
					<figure>
						<img src="/img/flyweight.png" alt="Flyweight UML Diagram" className="w-full h-auto rounded-md border border-gray-100 dark:border-gray-700 shadow-sm" loading="lazy" />
					</figure>
        	</div>
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-zinc-900 p-5 space-y-5">
          ...
          <details className="group rounded-lg border bg-white dark:bg-zinc-800 open:shadow-inner">
            <summary className="cursor-pointer select-none text-sm font-medium px-4 py-2 bg-gradient-to-r from-indigo-500/10 to-violet-500/10 dark:from-indigo-400/10 dark:to-violet-400/10 border-b flex items-center gap-2">
              <span className="text-indigo-600 dark:text-indigo-300 group-open:font-semibold">Ver interfaz, flyweight, factory, helper y cliente</span>
            ...
            <div className="p-4 space-y-4">
              <CodeBlock code={ifaceCode} title="flyweight-interface.tsx" />
              <CodeBlock code={concreteCode} title="concrete-flyweight.tsx" />
              <CodeBlock code={factoryCode} title="flyweight-factory.tsx" />
              <CodeBlock code={makeDataCode} title="helpers/make-data.ts" />
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-md border border-indigo-200 dark:border-indigo-800 bg-indigo-50/70 dark:bg-indigo-950/30 p-3 text-[12px] leading-relaxed text-indigo-900 dark:text-indigo-200">
        <strong>Cómo interactuar:</strong> Ajusta la cantidad de badges y observa que el contador de flyweights en caché se mantiene bajo (según variantes), mientras el total de items crece.
      </div>
      <FlyweightDemoRadix />
    </div>
  );
...
Playground.parameters = {
  docs: {
    description: {
      story: 'Flyweight comparte el estado intrínseco entre instancias para ahorrar memoria. En el ejemplo, las variantes comparten el estilo; el texto y la posición se pasan en cada render.'
    }
  }
};
