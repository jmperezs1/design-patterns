...
Widget createWrapper(Variant variant, Widget child) {
  ...
  if (variant == Variant.success) {
    style = Style(
      Colors.green.shade100, // bg-green-100
      Colors.green.shade800, // text-green-800
      Icons.check_circle_rounded,
    );
  } else if (variant == Variant.alert) {
    style = Style(
      Colors.red.shade100, // bg-red-100
      Colors.red.shade800, // text-red-800
      Icons.cancel_rounded,
    );
  } else if (variant == Variant.informative) {
    style = Style(
      Colors.blue.shade100, // bg-blue-100
      Colors.blue.shade800, // text-blue-800
      Icons.info_rounded,
    );
  } else if (variant == Variant.warning) {
    style = Style(
      Colors.yellow.shade100, // bg-yellow-100
      Colors.yellow.shade800, // text-yellow-800
      Icons.warning_amber_rounded,
    );
  } else {
    style = Style(
      Colors.grey.shade100, // bg-gray-100
