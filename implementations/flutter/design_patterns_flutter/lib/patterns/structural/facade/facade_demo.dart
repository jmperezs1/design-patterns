...
class _FacadeDemoState extends State<FacadeDemo> {
  ...
  void _setNowPlus(int hStart, int hEnd) {
    setState(() {
      _form['start'] = DateTime.now()
          .add(Duration(hours: hStart))
          .toIso8601String();
      _form['end'] = DateTime.now()
          .add(Duration(hours: hEnd))
          .toIso8601String();
    });
  }
    ...
    return Column(
      ...
      children: [
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  'El cliente llama a facade.book(). La fachada orquesta CRM, Calendario, Pagos y Notificaciones — y revierte en caso de fallo.',
                ...
                TextField(
                  ...
                  controller: TextEditingController(text: _form['email']),
                  onChanged: (v) => _form['email'] = v,
                ),
                const SizedBox(height: 6),
                TextField(
                  decoration: const InputDecoration(labelText: 'Teléfono'),
                  controller: TextEditingController(text: _form['phone']),
                  onChanged: (v) => _form['phone'] = v,
                ),
                const SizedBox(height: 6),
                TextField(
                  decoration: const InputDecoration(labelText: 'Recurso'),
                  controller: TextEditingController(text: _form['resourceId']),
                  onChanged: (v) => _form['resourceId'] = v,
                ),
                const SizedBox(height: 6),
                Row(
                  children: [
                    ElevatedButton(
                      onPressed: () => _setNowPlus(1, 2),
                      child: const Text('Ahora +1h → +2h'),
                    ),
                    const SizedBox(width: 8),
                    ElevatedButton(
                      onPressed: () => _setNowPlus(2, 3),
                      child: const Text('Ahora +2h → +3h'),
                    ),
                    const SizedBox(width: 8),
                    ElevatedButton(
                      onPressed: () => _setNowPlus(24, 25),
                      child: const Text('Mañana +1h → +2h'),
                    ),
                  ],
                ),
              ],
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Checkbox(
                      value: _simulateFail,
                      onChanged: (v) =>
                          setState(() => _simulateFail = v ?? false),
                    ),
                    const SizedBox(width: 8),
                ...
                Row(
                  children: [
                    ElevatedButton(
                      onPressed: (_busy ? null : _onBook),
                      child: Text(_busy ? 'Reservando…' : 'Reservar cita'),
                    ),
                    const SizedBox(width: 8),
                    OutlinedButton(
                      onPressed: () => setState(() {
        ...
        Container(
          ...
          child: _busy
              ? const Text('Ejecutando…')
              : _logs.isEmpty
              ? const Text('Aún no hay logs')
              : Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
