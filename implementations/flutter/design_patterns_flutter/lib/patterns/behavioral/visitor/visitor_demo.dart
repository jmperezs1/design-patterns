...
class _VisitorDemoState extends State<VisitorDemo> {
  ...
  void addItem() {
    ...
    el.Element it;
    if (kind == 'book')
      it = eb.Book(name, p, q, flag);
    else if (kind == 'electronics')
      it = ee.Electronics(name, p, q, flag);
    else
      it = eg.Grocery(name, p, q, flag);
  ...
  void calcTotals() {
    ...
    setState(
      () => totals = {'subtotal': v.subtotal, 'tax': v.tax, 'total': v.total},
    );
  }
      ...
      children: [
        Card(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Text(
                  'Visitor: Carrito con Operaciones',
                ...
                Text(
                  'Aplica distintos visitantes (totales, envío, exportación) sin modificar los elementos.',
                  style: Theme.of(
                    context,
        ...
        LayoutBuilder(
          builder: (context, c) {
                        ...
                        children: [
                          for (var i = 0; i < items.length; i++)
                            Padding(
                              ...
                              child: Row(
                                children: [
                                  Expanded(child: _renderItem(items[i])),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: calcTotals,
                          child: const Text('Calcular Totales'),
                        ),
                        OutlinedButton(
                          onPressed: calcShipping,
                          child: const Text('Estimar Envío'),
                        ),
                        OutlinedButton(
                          onPressed: exportCsv,
                    ...
                    if (totals != null)
                      Container(
                        ...
                        child: Text(
                          'Subtotal: \$${totals!['subtotal']!.toStringAsFixed(2)} · Impuestos: \$${totals!['tax']!.toStringAsFixed(2)} · Total: \$${totals!['total']!.toStringAsFixed(2)}',
                        ),
                      ),
                    if (shipping != null)
                      Container(
                        ...
                        child: Text(
                          'Envío estimado: \$${shipping!.toStringAsFixed(2)}',
                        ),
                      ),
                    if (csv.isNotEmpty)
                      Container(
            ...
            return Column(
              children: [itemsCard, const SizedBox(height: 12), opsCard],
            );
          },
        ),
      ],
  ...
  Widget _renderItem(el.Element it) {
    if (it is eb.Book)
      return Text(
        '📘 Libro: ${it.title} · \$${it.unitPrice} × ${it.qty} · ${it.isImported ? "Importado" : "Nacional"}',
      );
    if (it is ee.Electronics)
      return Text(
        '🔌 Electrónica: ${it.name} · \$${it.unitPrice} × ${it.qty} · ${it.fragile ? "Frágil" : "Normal"}',
      );
    if (it is eg.Grocery)
      return Text(
        '🥫 Alimento: ${it.name} · \$${it.unitPrice} × ${it.qty} · ${it.perishable ? "Perecedero" : "No perecedero"}',
      );
